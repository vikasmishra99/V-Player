...
import videoplayer.hdvideoplayer.xvideoplayer.mp3musicplayer.fullhdvideoplayer.extensions.ExtensionsManager
import org.videolan.vlc.extensions.api.VLCExtensionItem
import videoplayer.hdvideoplayer.xvideoplayer.mp3musicplayer.fullhdvideoplayer.gui.HistoryFragment
...
import videoplayer.hdvideoplayer.xvideoplayer.mp3musicplayer.fullhdvideoplayer.gui.browser.FileBrowserFragment
import videoplayer.hdvideoplayer.xvideoplayer.mp3musicplayer.fullhdvideoplayer.gui.browser.NetworkBrowserFragment
import videoplayer.hdvideoplayer.xvideoplayer.mp3musicplayer.fullhdvideoplayer.gui.network.MRLPanelFragment
...
import videoplayer.hdvideoplayer.xvideoplayer.mp3musicplayer.fullhdvideoplayer.gui.view.HackyDrawerLayout
import videoplayer.hdvideoplayer.xvideoplayer.mp3musicplayer.fullhdvideoplayer.util.*
import videoplayer.hdvideoplayer.xvideoplayer.mp3musicplayer.fullhdvideoplayer.viewmodels.mobile.VideoGroupingType
...
class Navigator: NavigationView.OnNavigationItemSelectedListener, LifecycleObserver, INavigator {
    ...
    private fun getTag(id: Int) = when (id) {
        ...
        R.id.nav_music -> ID_MUSIC_PLAYER
        R.id.nav_play_apps -> ID_MORE_APPS
        R.id.share_player -> ID_SHARE_APP
            ...
            if (currentFragmentId == id) { /* Already selected */
                ...
                R.id.nav_music ->{
                    try{
                        ...
                        this.activity.startActivity(playstoreIntent1)
                    }catch (exp:Exception){
                        ...
                        this.activity.startActivity(playstoreIntent2)
                    }
                ...
                R.id.nav_play_apps ->{
                    try{
                        var playstoreuri3:Uri=Uri.parse("market://search?q=pub:Video+Downloader+-+All+Downloader+%26+Video+Players")
                        var playstoreIntent3: Intent = Intent(Intent.ACTION_VIEW, playstoreuri3)
                        this.activity.startActivity(playstoreIntent3)
                    }catch (exp:Exception){
                        var playstoreuri4: Uri = Uri.parse("http://play.google.com/store/search?q=pub:Video+Downloader+-+All+Downloader+%26+Video+Players")
                        var playstoreIntent4: Intent = Intent(Intent.ACTION_VIEW, playstoreuri4)
                        this.activity.startActivity(playstoreIntent4)
                    }
                ...
                // share app
                R.id.share_player ->{
                    val sharebody = this.activity.getString(R.string.share_desc) + " \n " + " \n " + "https://play.google.com/store/apps/details?id=" + this.activity.packageName
                    val shareIntent = Intent()
                    shareIntent.action = Intent.ACTION_SEND
                    shareIntent.type="text/plain"
                    shareIntent.putExtra(Intent.EXTRA_TEXT, sharebody)
                    this.activity.startActivity(Intent.createChooser(shareIntent, "Select App to Share Video Player.."))
                }
