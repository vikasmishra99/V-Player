...
import videoplayer.hdvideoplayer.xvideoplayer.mp3musicplayer.fullhdvideoplayer.util.UPDATE_DESCRIPTION
...
class NetworkBrowserFragment : MediaSortedFragment<NetworkModel>() {
    ...
    override fun onStart() {
        super.onStart()
        LocalBroadcastManager.getInstance(VideoPlayerApplication.appContext).registerReceiver(localReceiver, IntentFilter(VlcLoginDialog.ACTION_DIALOG_CANCELED))
    }
    ...
    override fun onStop() {
        super.onStop()
        LocalBroadcastManager.getInstance(VideoPlayerApplication.appContext).unregisterReceiver(localReceiver)
    }
