...
import videoplayer.hdvideoplayer.xvideoplayer.mp3musicplayer.fullhdvideoplayer.databinding.PlaylistItemBinding
...
import videoplayer.hdvideoplayer.xvideoplayer.mp3musicplayer.fullhdvideoplayer.util.MediaItemDiffCallback
import videoplayer.hdvideoplayer.xvideoplayer.mp3musicplayer.fullhdvideoplayer.util.WeakHandler
...
class PlaylistAdapter(private val player: IPlayer) : DiffUtilAdapter<AbstractMediaWrapper, PlaylistAdapter.ViewHolder>(), SwipeDragHelperAdapter {
    ...
    override fun onItemDismiss(position: Int) {
        val media = getItem(position)
        val message = String.format(VideoPlayerApplication.appResources.getString(R.string.remove_playlist_item), media.title)
        if (player is Fragment) {
            ...
            Toast.makeText(VideoPlayerApplication.appContext, message, Toast.LENGTH_SHORT).show()
        }
        remove(position)
    }
