...
import videoplayer.hdvideoplayer.xvideoplayer.mp3musicplayer.fullhdvideoplayer.databinding.EqualizerBinding
import videoplayer.hdvideoplayer.xvideoplayer.mp3musicplayer.fullhdvideoplayer.gui.dialogs.VLCBottomSheetDialogFragment
...
import videoplayer.hdvideoplayer.xvideoplayer.mp3musicplayer.fullhdvideoplayer.interfaces.OnEqualizerBarChangeListener
import videoplayer.hdvideoplayer.xvideoplayer.mp3musicplayer.fullhdvideoplayer.util.Settings
import videoplayer.hdvideoplayer.xvideoplayer.mp3musicplayer.fullhdvideoplayer.util.VLCInstance
import videoplayer.hdvideoplayer.xvideoplayer.mp3musicplayer.fullhdvideoplayer.util.VLCOptions
...
class EqualizerFragment : VLCBottomSheetDialogFragment(), CoroutineScope by MainScope() {
    ...
    private val newPresetName = VideoPlayerApplication.appResources.getString(R.string.equalizer_new_preset_name)
    ...
    private fun createSaveCustomSetDialog(positionToSave: Int, displayedByUser: Boolean, onPause: Boolean) {
        ...
        val saveEqualizer = AlertDialog.Builder(requireActivity())
                ...
                .setNegativeButton(R.string.do_not_save) { _, _ ->
                    if (onPause)
                        VLCOptions.saveEqualizerInSettings(VideoPlayerApplication.appContext, equalizer, allSets[positionToSave], binding.equalizerButton.isChecked, false)
                }
                .setOnCancelListener {
                    if (onPause)
                        VLCOptions.saveEqualizerInSettings(VideoPlayerApplication.appContext, equalizer, allSets[positionToSave], binding.equalizerButton.isChecked, false)
    ...
    private fun save(input: EditText, oldName: String, temporarySet: MediaPlayer.Equalizer, onPause: Boolean, displayedByUser: Boolean, positionToSave: Int, saveEqualizer: AlertDialog) {
        ...
        if (newName.contains("_") || TextUtils.equals(newName, newPresetName)) {
            Toast.makeText(ctx, VideoPlayerApplication.appContext.resources.getString(R.string.custom_set_wrong_input), Toast.LENGTH_SHORT).show()
        } else if (allSets.contains(newName) && !TextUtils.equals(newName, oldName)) {
            Toast.makeText(ctx, VideoPlayerApplication.appContext.resources.getString(R.string.custom_set_already_exist), Toast.LENGTH_SHORT).show()
        } else {
