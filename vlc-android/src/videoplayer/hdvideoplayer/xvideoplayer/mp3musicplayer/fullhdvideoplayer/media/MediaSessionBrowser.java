...
import videoplayer.hdvideoplayer.xvideoplayer.mp3musicplayer.fullhdvideoplayer.util.SettingsKt;
...
public class MediaSessionBrowser implements ExtensionManagerService.ExtensionManagerActivity {
    ...
    public void displayExtensionItems(int extensionId, String title, List<VLCExtensionItem> items, boolean showParams, boolean isRefresh) {
        ...
        for (int i=0; i<items.size(); i++) {
            ...
            if (coverUri == null)
                mediaItem.setIconBitmap(UiTools.INSTANCE.getDefaultAudioDrawable(VideoPlayerApplication.Companion.getAppContext()).getBitmap());
            else
