...
import videoplayer.hdvideoplayer.xvideoplayer.mp3musicplayer.fullhdvideoplayer.gui.dialogs.SubtitleItem
import videoplayer.hdvideoplayer.xvideoplayer.mp3musicplayer.fullhdvideoplayer.gui.helpers.hf.getExtWritePermission
...
object VLCDownloadManager: BroadcastReceiver(), LifecycleObserver {
    private val downloadManager = VideoPlayerApplication.appContext.getSystemService(Context.DOWNLOAD_SERVICE) as DownloadManager
    ...
    fun register() {
        VideoPlayerApplication.appContext.applicationContext.registerReceiver(this, IntentFilter(DownloadManager.ACTION_DOWNLOAD_COMPLETE))
    }

    @OnLifecycleEvent(Lifecycle.Event.ON_DESTROY)
    fun unRegister() {
        ExternalSubRepository.getInstance(VideoPlayerApplication.appContext).downloadingSubtitles.observeForever {
            it?.keys?.forEach {
                downloadManager.remove(it)
            }
        }

        VideoPlayerApplication.appContext.applicationContext.unregisterReceiver(this)
    }
