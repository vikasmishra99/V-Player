...
import videoplayer.hdvideoplayer.xvideoplayer.mp3musicplayer.fullhdvideoplayer.gui.helpers.hf.WriteExternalDelegate
...
object Util {
    ...
    fun readAsset(assetName: String, defaultS: String): String {
        ...
        try {
            inputStream = VideoPlayerApplication.appResources.assets.open(assetName)
    ...
    fun byPassChromecastDialog(dialog: Dialog.QuestionDialog): Boolean {
        if ("Insecure site" == dialog.title) {
            ...
            Toast.makeText(VideoPlayerApplication.appContext, R.string.cast_performance_warning, Toast.LENGTH_LONG).show()
            dialog.postAction(1)
            dialog.dismiss()
            return true
        }
