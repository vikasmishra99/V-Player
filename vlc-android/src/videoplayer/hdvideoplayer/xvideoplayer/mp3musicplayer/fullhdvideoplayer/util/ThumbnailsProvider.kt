...
import videoplayer.hdvideoplayer.xvideoplayer.mp3musicplayer.fullhdvideoplayer.gui.helpers.AudioUtil.readCoverBitmap
...
object ThumbnailsProvider {
    ...
    fun getMediaCacheKey(isMedia: Boolean, item: MediaLibraryItem): String? {
        if (isMedia && (item as AbstractMediaWrapper).type == AbstractMediaWrapper.TYPE_VIDEO && TextUtils.isEmpty(item.getArtworkMrl())) {
            if (appDir == null) appDir = VideoPlayerApplication.appContext.getExternalFilesDir(null)
    ...
    private suspend fun composePlaylistImage(mediaList: List<AbstractMediaWrapper>, width: Int): Bitmap? {
        ...
        for (i in 0..3) {
            if (images.size < i + 1) {
                images.add(UiTools.getDefaultAudioDrawable(VideoPlayerApplication.appContext).bitmap)
            }
        }
