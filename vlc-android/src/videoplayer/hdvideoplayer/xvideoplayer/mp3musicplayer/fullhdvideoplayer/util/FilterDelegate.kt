        ...
        if (charSequence !== null) initSource()?.let { list ->
            return withContext(Dispatchers.Default) { mutableListOf<AbstractMediaWrapper>().apply {
                ...
                for (media in list) {
                    ...
                    val artist = MediaUtils.getMediaArtist(VideoPlayerApplication.appContext, media).toLowerCase()
                    val albumArtist = MediaUtils.getMediaAlbumArtist(VideoPlayerApplication.appContext, media).toLowerCase()
                    val album = MediaUtils.getMediaAlbum(VideoPlayerApplication.appContext, media).toLowerCase()
                    val genre = MediaUtils.getMediaGenre(VideoPlayerApplication.appContext, media).toLowerCase()
